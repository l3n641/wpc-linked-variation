        ...
        if (!class_exists('WPCleverWpclv')) {
            class WPCleverWpclv
            {
                            ...
                            <div class="wpclv_td">
                                <label> <select class="wpclv-source" name="wpclv_link[source]">
                                        ...
                                        foreach ($taxonomies as $taxonomy) {
                                            ...
                                            echo '<option value="' . esc_attr($taxonomy->name) . '" ' . ($link_source === $taxonomy->name ? 'selected' : '') . " >" . esc_html($taxonomy->label) . '</option>';
                                        }
                                        ?>
                                    </select> </label>
                            </div>
                ...
                public static function get_linked_products($link_data, $context = 'default')
                {
                    ...
                    $order_by = $link_data['orderby'] === "default" ? "id" : $link_data['orderby'];
                    $order = $link_data['order'] === "default" ? "DESC" : "ASC";
                    ...
                    if (!self::enable_cache('products') || (false === ($link_products = get_transient('wpclv_linked_products_' . $link_id)))) {
                        ...
                        } elseif (($link_source === 'product_collection')) {
                            if (empty($link_data['terms'])) {
                                ...
                                foreach ($collections as $collection) {
                                    $term_names[] = $collection->name;
                                }
                            } else {
                                $term_names = explode(',', $link_data['terms']);
                            }

                            $products = wpc_get_wc_products_by_term_names($term_names, $link_data['limit'], $order_by, $order);
                            if ($products) {
                                foreach ($products as $product) {
                                    $link_products[] = $product->get_id();
                                }
                            }
                        }
...
add_action('init', 'wpc_register_product_product_collection_taxonomy');

function wpc_register_product_product_collection_taxonomy()
{
    // 定义插件文本域（用于多语言翻译）
    $domain = 'wpc-linked-variation';

    register_taxonomy(
        'product_collection', // 分类法 Slug
        'product',            // 绑定到 WooCommerce 商品
        [
            'labels' => [
                'name' => __('Album Management', $domain),
                'singular_name' => __('Album Name', $domain),
                'menu_name' => __('Albums', $domain),
                'add_new_item' => __('Add New Album', $domain),
                'search_items' => __('Search Albums', $domain),
                'all_items' => __('All Albums', $domain),
                'edit_item' => __('Edit Album', $domain),
                'update_item' => __('Update Album', $domain),
                'new_item_name' => __('New Album Name', $domain),
            ],
            'hierarchical' => false, // 像标签一样扁平化管理
            'show_ui' => true,  // 在后台显示界面
            'show_admin_column' => true,  // 在商品列表中直接展示这一列，方便运营和跨境查看
            'show_in_rest' => true,  // 支持 REST API（完美兼容 ERP 接口读写）
        ]
    );
}
...
/**
 * 根据专辑名称获取 WooCommerce 商品对象数组
 * * @param string[] $collection_names 专辑名称数组
 * @return WC_Product[] 商品对象数组
 */
function wpc_get_wc_products_by_term_names($term_names, $limit = 100, $orderby = 'id', $order = 'DESC'): array
{
    // 1. 如果传入的是字符串，自动转换为数组，提高函数的兼容性
    $term_names = (array)$term_names;

    // 2. 使用 get_terms 一次性查询所有对应的 term ID
    $terms = get_terms([
    ...
    // 提取出所有的 term_id 组成一个数组
    $term_ids = wp_list_pluck($terms, 'term_id');

    // 3. 使用 wc_get_products 进行查询
        ...
        'custom_query' => [
            'tax_query' => [
                [
                    ...
                    'field' => 'term_id',
                    'terms' => $term_ids,
                    'operator' => 'IN', //IN 表示只要商品属于其中任意一个集合就查出来（默认）
                ],
            ],
        ],
    ]);
