...
(function ($) {
    ...
    $(document).on('change', '.wpclv-product-search', function () {
        ...
        if (Array.isArray(_val)) {
            $(this).closest('.wpclv_link').find('.wpclv-products').val(_val.join()).trigger('change');
        } else {
            if (_val === null) {
                $(this).closest('.wpclv_link').find('.wpclv-products').val('').trigger('change');
            } else {
                $(this).closest('.wpclv_link').find('.wpclv-products').val(String(_val)).trigger('change');
            }
        }
    });
    ...
    $(document).on('change', '.wpclv-category-search', function () {
        ...
        if (Array.isArray(_val)) {
            $(this).closest('.wpclv_link').find('.wpclv-categories').val(_val.join()).trigger('change');
        } else {
            if (_val === null) {
                $(this).closest('.wpclv_link').find('.wpclv-categories').val('').trigger('change');
            } else {
                $(this).closest('.wpclv_link').find('.wpclv-categories').val(String(_val)).trigger('change');
            }
        }
    });
    ...
    $(document).on('change', '.wpclv_display_checkbox', function () {
        if ($(this).prop('checked')) {
            $(this).closest('.wpclv-attribute').find('.wpclv_display_checkbox').not(this).prop('checked', false);
        }
    });
    ...
    $(document).on('change', '.wpclv-terms-select', function () {
        ...
        if (Array.isArray(val)) {
            $this.closest('.wpclv_link').find('.wpclv-terms-val').val(val.join()).trigger('change');
        } else {
            if (val === null) {
                $this.closest('.wpclv_link').find('.wpclv-terms-val').val('').trigger('change');
            } else {
                $this.closest('.wpclv_link').find('.wpclv-terms-val').val(String(val)).trigger('change');
            }
        }
    ...
    function wpclv_source($this) {
        ...
        if (source === 'products' || source === 'categories' || source === 'tags') {
            ...
            $this.closest('.wpclv_link').find('.wpclv-source-terms-label').text(label);
